...
export default function Home() {
  ...
  const updateInventory = async () => {
    ...
    try {
      const snapshot = query(collection(firestore, 'demo_pantry_items'));
      ...
      console.error("Failed to fetch Firebase data, falling back to mock mode.");
      setInventory([]);
  ...
  const addItem = async (item) => {
    if (useMock) {
      ...
      if (existing) {
        ...
        newInventory = [...inventory, { name: item, quantity: 1 }];
      }
    ...
    try {
      const docRef = doc(collection(firestore, 'demo_pantry_items'), item);
      ...
      console.warn("Failed to add item to Firebase. Falling back to local state.");
      const existing = inventory.find(i => i.name === item);
      let newInventory;
      if (existing) {
        newInventory = inventory.map(i =>
  ...
  const removeItem = async (item) => {
    ...
    try {
      ...
      console.warn("Failed to remove item from Firebase. Falling back to local state.");
      const newInventory = inventory.map(i =>
        i.name === item ? { ...i, quantity: i.quantity - 1 } : i
      ).filter(i => i.quantity > 0);
      setInventory(newInventory);
      ...
      >
       ...
       <Stack width='100%' height='300px' spacing={2} overflow='auto'>
        {filteredInventory.map(({name, quantity}) => (
          ...
          >
            <Typography variant = 'h4' color = "#333" textAlign='left' sx={{ width: '40%', minWidth: '200px' }}>
              {name.charAt(0).toUpperCase() + name.slice(1)}
            </Typography>
            <Typography variant = 'h4' color = "#333" textAlign='center'>
              {quantity}
            </Typography>
            <Stack direction='row' spacing={2} sx={{ width: '200px', justifyContent: 'flex-end' }}>
